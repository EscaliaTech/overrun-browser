...
| D-017 | Click-through del overlay | **Toggle manual**, pendiente de soporte de arquitectura: `BaseWindow.setIgnoreMouseEvents` afecta toda la ventana y no sirve para un único `WebContentsView`. Requiere overlay transparente en ventana separada para interactuar con la página debajo sin moverlo | REQ-013 |
